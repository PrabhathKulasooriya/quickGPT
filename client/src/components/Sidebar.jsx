...
const Sidebar = ({isMenuOpen, setIsMenuOpen}) => {
  ...
  return (
    ...
    backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-10 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      {/* New Chat Button*/}
      <button
        className="flex justify-center items-center w-full py-2 mt-10 text-white bg-gradient-to-r from-[#A456F7] to-[#3D81F6] 
          ...
          .map((chat) => (
            <div
              ...
              onClick = {()=>{navigate('/');
                              ...
                              setIsMenuOpen(false);}}
              className="p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer
      ...
      {/* User Account */}
      <div
          className="flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15
          ...
          {user && <img src={assets.logout_icon} className='h-5 cursor-pointer md:hidden not-dark:invert group-hover:block'onClick={()=>alert('logout')}/>}
      </div>
